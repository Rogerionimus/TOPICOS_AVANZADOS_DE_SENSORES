...
void loop() {
  ...
  if (a2[0][0] <= 0 && a2[1][0] <= 0) {
    ...
    cara_actual = "Sensores";
  } else if(a2[0][0] > 0 && a2[1][0] <= 0) {
    cara_actual = "Neumatica";
  } else {
    cara_actual = "Automatas";
  }
  ...
  unsigned long tiempo_transcurrido = (millis() - tiempo_inicio) / 1000; 

  // Conversión a formato HH:MM:SS
  int horas = tiempo_transcurrido/3600;
  int minutos = (tiempo_transcurrido % 3600) / 60;
  int segundos = tiempo_transcurrido % 60;
  ...
  // Cronómetro con formato de hora, minuto, segundo
  display.setTextSize(1);
  display.setCursor(0, 50);
  display.print("Tiempo: ");

  // Formato con ceros a la izquierda (00:00:00)
